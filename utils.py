...
class Load_Rumours_Dataset_filtering_since_first_post:
    ...
    def process_data(self):
        ...
        #scaled_data['id'] = train['id'].values
        ...
        test_val_df_replies['min_since_fst_post_test'] = round(
           (test_val_df_replies['time'] - test_val_df_replies['time'].min()).dt.total_seconds() / 60, 2)

        test_val_df_replies['reply_min_since_fst_post_test'] = round(
           (test_val_df_replies['reply_time'] - test_val_df_replies['reply_time'].min()).dt.total_seconds() / 60, 2)

        # Filter by time_cut
        test_val_df_replies = test_val_df_replies[
            (test_val_df_replies.reply_min_since_fst_post_test <= self.time_cut) &
            (test_val_df_replies.min_since_fst_post_test <= self.time_cut)]

        test_val_df_replies_to_filter = test_val_df_replies[
            (test_val_df_replies.min_since_fst_post_test <= self.time_cut)][['id']].drop_duplicates()


        # Group replies again
        grouped_replies = test_val_df_replies.groupby(['id', 'min_since_fst_post_test']).agg(
            replies=('time_diff', 'count'),
            first_time_diff=('time_diff', 'first')
        ).reset_index()
        ...
        test_val_df_posts = test_val_df_posts[features].merge(grouped_replies, on="id", how="left").fillna(0)
        test_val_df_posts = test_val_df_posts[test_val_df_posts.id.isin(test_val_df_replies_to_filter.id)]
        test_val_df_posts =test_val_df_posts.merge(self.df_posts[['id','min_since_fst_post']], on="id", how="inner").fillna(0)

        test_val_df_posts.sort_values(by='min_since_fst_post_test',ascending=True,inplace=True)
        ...
        if 'no_verified' not in test_val_df_posts.columns:
            test_val_df_posts['no_verified'] = (
                test_val_df_posts['verified'] == 0
            ).astype(int)

        if 'verified' not in test_val_df_posts.columns:
             test_val_df_posts['verified'] = (
                test_val_df_posts['no_verified'] == 0
            ).astype(int)
...
class Load_Rumours_Dataset_filtering_since_first_post_Transfer_Learning:
    ...
    def process_data(self):
        ...
        df_post_time = self.df_replies[['time','id']].drop_duplicates().sort_values(by='time',ascending=True)
        ...
        post_features = df_posts_train[[
            ...
            "verified", "rumour", "embeddings_avg", "replies", "first_time_diff", "min_since_fst_post","id"
        ]]
        ...
        scaled_data['rumour'] = post_features['rumour'].values
        scaled_data['min_since_fst_post'] = post_features['min_since_fst_post'].values
        scaled_data['id'] = post_features['id'].values
        ...
        test_val_df_replies_to_filter = test_val_df_replies[
            (test_val_df_replies.min_since_fst_post <= self.time_cut)][['id']].drop_duplicates()
        ...
        test_val_df_posts = test_val_df_posts[features].merge(grouped_replies, on="id", how="left")
        test_val_df_posts = test_val_df_posts[test_val_df_posts.id.isin(test_val_df_replies_to_filter.id)]
        ...
        post_features = test_val_df_posts[[
            "followers", "favorite_count", "retweet_count", "no_verified", "verified","id",
            "replies", "first_time_diff", "embeddings_avg", "rumour", "min_since_fst_post"
        ]]
...
class Hetero_Data_Processor_Filter_on_Test_since_first_post:
    ...
    def process_data(self):
        ...
        test_val_df_posts = test_val_df_posts[post_features].merge(grouped_replies, on="id", how="left").fillna(0)
        test_val_df_posts = test_val_df_posts[test_val_df_posts.id.isin(test_val_df_replies_to_filter.id)]
